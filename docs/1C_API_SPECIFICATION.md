...
| `asOfDate` | string | **🆕 Опціональний.** Дата у форматі YYYY-MM-DD. Якщо передано — факт рахується з 1-го числа місяця по `asOfDate` включно. Якщо не передано або null — береться **останній день місяця `period`** (тобто повний місяць). Використовується для порівняння з минулим місяцем на той самий N-й робочий день. |
...
| `asOfDate` | string | **🆕 Опціональний.** Дата у форматі YYYY-MM-DD. Якщо передано — факт поточного місяця рахується по цю дату включно, факт минулого місяця — по той самий N-й робочий день минулого місяця (для коректного порівняння динаміки). Якщо не передано — береться **останній день місяця `period`** (повний місяць). |
...
**Важливо:**
- Факт — з 1-го числа місяця по `asOfDate` включно (або по останній день місяця `period` якщо `asOfDate` не передано)
