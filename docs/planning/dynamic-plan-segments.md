...
## 11. Розрахунок «План» / «Заплановано» / «Запланований %» з dynamic (2026-07-08)

Ключове: **dynamic-сегмент (NEURONOX) входить у ОБИДВА числа однаково — як ФАКТ (дзеркало)**, тому сам по собі дає рівно 100% і НЕ спотворює відсоток.

**«План місяця» / «План регіону»** (`effectiveTotalPlan` — region-accordion / rm / director / manager):
```
План = Σ по сегментах:  dynamic → factAmount (дзеркало)  ·  інакше → planAmount (1С)
```

**«Заплановано»** (`finalizedExpected` / `regionExpectedAmount` — те що менеджери ЗАФІКСУВАЛИ, тільки finalized, чернетки не в рахунок):
```
Заплановано = Σ (forecastFinalized + gapFinalized) по НЕ-dynamic сегментах
            + Σ factAmount по dynamic сегментах (дзеркало)
```
- `forecast` = блок «Прогноз» (активні клієнти), `gap` = «Закриття розриву».

**«Запланований %»** = `Заплановано / План × 100`.

**Чому % буває >100%:** через ЗВИЧАЙНІ бренди — менеджери закомітили (прогноз+розрив) більше, ніж 1С-план по них. NEURONOX (N) один і той самий у чисельнику й знаменнику: План = 1С(звич)+N, Заплановано = коміт(звич)+N.

⚠️ **Всі місця мають рахувати однаково.** Була розбіжність: зовнішній рядок регіону у директора (`regionExpectedAmount`) рахував сиру `Σ(forecast+gap)` включно з введеним по NEURONOX, а hero регіону — з дзеркалом (факт). Виправлено 2026-07-08 (commit `8853196`) — рядок теж пропускає dynamic forecast+gap і додає факт регіону.

### 11.1 Додатковий рядок «Без динамічного бренду» (region-accordion) — ✅ у проді 2026-07-08 (tag `etalon-2026-07-08`)
Під головним рядком регіону (перед mini-list менеджерів) показуємо ДРУГИЙ рядок — ті самі «Запл. %·сума / План», але **повністю БЕЗ dynamic-сегментів** (тільки звичайні бренди):
```
План(без дин)       = Σ planAmount по НЕ-dynamic сегментах (1С)
Заплановано(без дин) = Σ (forecastFinalized+gapFinalized) по НЕ-dynamic сегментах
% (без дин)          = Заплановано(без дин) / План(без дин) × 100
```
Показується лише коли регіон реально має dynamic-сегмент (інакше = головні числа, дубль).
