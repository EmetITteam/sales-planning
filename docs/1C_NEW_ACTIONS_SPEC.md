...
### 2.4 Pagination (рішення: без пагінації, клієнт ділить період)

**Зафіксовано 2026-06-03 після уточнення з 1С-розробником.**

**Усі batch-actions реалізуйте БЕЗ пагінації.** Документуйте ліміт **до 10 000 рядків за виклик**. Якщо клієнтський запит з широким періодом виходить за цей ліміт — frontend сам розіб'є період на менші шматки (наприклад, помісячно для бекфілу `getDetailedSalesBatch`).

**Чому так:**

- У 1С нема нативного OFFSET/LIMIT — лише `ВЫБРАТЬ ПЕРВЫЕ N`. Offset-pagination через пропуск рядків неефективна (1С вичитує і пропускає перші N×M рядків — ресурси у трубу)
- Для нашого реального use-case (бекфіл sales-detail помісячно) 1 місяць ≈ 5-10k рядків — поміщається у одну відповідь (JSON ~400 байт/рядок → 2-4 МБ)
- Простіший контракт для розробника, простіша синхронізація для cron-worker

**Якщо у production побачимо що 1 місяць виходить за 10k рядків — додамо keyset cursor pagination** (НЕ offset) у наступній ітерації:

```json
// Майбутній формат (поки не реалізуємо):
// payload: { ..., "cursor": "REAL-2026-001234:5" }
// response.data.nextCursor + response.data.hasMore
```

Стандартний формат відповіді **без пагінації:**
...
**1С запит без пагінації (приклад для `getDetailedSalesBatch`):**

```
Запрос.Текст = 
    "ВЫБРАТЬ
    |    Реализация.Номер КАК doc_number_1c,
    |    Реализация.Дата КАК doc_date,
    |    ...
    ...
    |ГДЕ
    |    Реализация.Дата МЕЖДУ &fromDate И &toDate
    |    И Реализация.Проведен
    |УПОРЯДОЧИТЬ ПО
    |    Реализация.Дата, Реализация.Номер, Товары.НомерСтроки";
```
