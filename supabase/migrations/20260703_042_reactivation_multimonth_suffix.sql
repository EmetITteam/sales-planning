-- ============================================================================
-- Migration 042: Reactivation — no month suffix for multi-month periods
-- Created 2026-07-03
-- ============================================================================
--
-- ПРОБЛЕМА: для періоду > 1 місяця (квартал/півріччя/рік) base_name приклеював
-- суфікс МІСЯЦЯ p_from — тож червневий «Collagen» у піврічному звіті показувався
-- як «(01.26)» (підміна місяця). Акції все одно треба схлопувати за базовою
-- назвою, але БЕЗ конкретного місяця.
--
-- ФІКС: v_suffix = MM.YY тільки якщо період рівно 1 місяць (p_to = p_from + 1
-- month). Інакше NULL → суфікс не додаємо (акції схлопуються за базовою назвою).
--
-- ⚠️ Додано DECLARE v_suffix; змінено вираз base_name у rows_full. Решта — з 041.
-- ============================================================================
...
COMMENT ON FUNCTION get_reactivation_analytics(TEXT, TIMESTAMPTZ, TIMESTAMPTZ) IS
  'Реактивація v9: суфікс місяця тільки для 1-місячного періоду (для квартал/рік акції схлопуються без місяця, без підміни). Решта — v8 (041).';
