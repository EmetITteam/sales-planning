-- ============================================================================
-- Migration 059: ВІДНОВЛЕННЯ get_reactivation_analytics до v9 (міграція 042)
-- Created 2026-07-17
-- ============================================================================
--
-- ⚠️ Міграція 057 помилково перезаписала функцію старою версією (032) — це
-- скинуло усі доробки 033-042 (representatives-only з 040, __cat_total__ з 035,
-- promo-merge/gift-reattribution/suffix) і зламало борд («category is
-- ambiguous», 42702; representatives-фільтр там і так уже був з 040).
--
-- Ця міграція повертає ТОЧНУ v9-версію з 042 (verbatim). Канал representatives
-- уже всередині (period_buyers/period_rows_ng/rows_full). Історія класифікації
-- свідомо НЕ фільтрується по каналу (як в оригіналі).
-- ============================================================================
...
AS $$
DECLARE
  -- Суфікс місяця показуємо ТІЛЬКИ для рівно одномісячного періоду.
  v_suffix TEXT := CASE
    WHEN p_to = p_from + INTERVAL '1 month'
      THEN to_char(p_from AT TIME ZONE 'UTC', 'MM.YY')
    ELSE NULL
  END;
BEGIN
  RETURN QUERY
...
COMMENT ON FUNCTION get_reactivation_analytics(TEXT, TIMESTAMPTZ, TIMESTAMPTZ) IS
  'Реактивація v9 (відновлено після 057). Канал representatives, __cat_total__, promo-merge/gift/suffix. Суфікс місяця тільки для 1-місячного періоду.';
