/**
 * Збірка Зведеного звіту РОП (Лист 4) — усі 8 представництв в одному об'єкті.
 * Винесено з GET /api/rop-report щоб переиспользувати з xlsx-експорту.
 *
 * СЕРВЕРНА агрегація ПОВЕРХ існуючих розрахунків (getRegionData, aggregateRegion,
...
export type BuildRopResult =
  | { ok: true; data: RopReport }
  | { ok: false; error: string; status: number };

/**
 * Збирає Зведений звіт РОП для періоду 'YYYY-MM' на дату тижня.
 * Виконує ВСЕ що робив GET-хендлер ПІСЛЯ авторизації.
 * Ранні помилки (поганий період / нема тижнів / збій 1С/БД) → { ok:false }.
 */
export async function buildRopReport(
  period: string,
  weekParam: string | null,
  viewerLogin: string,
): Promise<BuildRopResult> {
  const today = new Date().toISOString().slice(0, 10);
  if (!/^\d{4}-\d{2}$/.test(period)) return { ok: false, error: 'period must be YYYY-MM', status: 400 };
  ...
  if (weeks.length === 0) return { ok: false, error: 'no weeks in period', status: 400 };
  const week = weekParam && weeks.some(w => w.weekEnd === weekParam)
    ? weekParam
  ...
  if (!regResp.ok || !regResp.data) {
    return { ok: false, error: `getRegionData: ${regResp.errorMessage ?? 'no data'}`, status: 502 };
  }
  ...
  if (allLogins.length > 0) {
    ...
    if (error) return { ok: false, error: `period_summaries: ${error.message}`, status: 500 };
