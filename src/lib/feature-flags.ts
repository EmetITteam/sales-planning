...
/**
 * Логіни, що НЕ ведуть клієнтів і не планують — виключаємо зі СПИСКУ МЕНЕДЖЕРІВ
 * регіону скрізь (планування, звіт РМ, зведений звіт РОП). 1С повертає їх у
 * managers[] бо вони у ноді регіону, але вони не мають бути рядком-менеджером.
 *
 * headofsd (Мігашко) — РОП власних представництв: керує, але сама не продає/не
 * планує (0 рядків у period_summaries). Без цього регіон Київ фейково «чернетка»
 * і вона зайвим рядком псує планування/звіт (узгоджено 2026-07-22).
 */
export const NON_PLANNING_MANAGERS: readonly string[] = ['headofsd@emet.in.ua'];
