...
## [2026-07-17] · Тижневий звіт — фіналізація звіту (як фіналізація планів)

### ✅ Фіналізація звіту (РМ)

- Внизу борду — кнопка **«Фіналізувати звіт»**. Перед фіналізацією перевіряє повноту: по кожному бренду **Причина + Дія**, **Висновок** по регіону, і всі **прошлотижневі обіцянки** відмічені (виконано/ні). Якщо чогось бракує — показує список «Ще не заповнено», кнопка неактивна.
- Після фіналізації — стан «Звіт фіналізовано» (ким/коли) + **«Пере-відкрити»** (з підтвердженням).

### 👁️ Зведення для директора/РОП

- Вгорі сторінки (лише оверсайт-ролі) — картка **«Фіналізація звітів · тиждень»**: скільки регіонів здали звіт / скільки ні, дрилдаун по регіонах (ким/коли). Аналог «Готовності планування».

### 🔀 Дрібне

- Кнопки на картці бренда: **Причина → Дія** (спершу причина результату, потім дія).

### 🗄️ Міграція

- `056_weekly_report_status` — фіналізація звіту (регіон × тиждень, `finalized_at/by`).

---

## [2026-07-16] · Тижневий звіт (паспорт наради РОП–РМ) + live-синк продажів

### 📋 Нова сторінка «Тижневий звіт» (`/weekly-report`)

Авто-паспорт наради РОП–РМ: числа з борду + ручні замітки менеджера. Доступ через account-меню.

- **Паспорт регіону** — hero-картки (стиль борду `MetricCard`): Виконання, Розрив на сьогодні (норма-на-дату), Прогноз темпу, Минулий місяць, + Заплановано (фіналізовано). Розрив рахується **на поточний день** (план × пройдені роб.дні / усі), а не за весь місяць.
- **Розклад по категоріях клієнтів (1С)** — по кожній 1С-категорії (Активні/Сплячі/Втрачені/Нові/Без закупок) **унікальні** клієнти: Заплановано → База → % → Купили → % з запл. → % з бази. Дрилдаун по менеджерах. Резерв виключається.
- **По брендах** — план/факт/%/мітка + Прогноз (темп)/Запл./Мин.міс. + чіпи «заплановано→купили» по категоріях + кнопки Дія/Причина.
- **Замітки Дія / Причина / Висновок** — зберігаються понедельно з **історією версій** (append-only). «Обіцяв минулого тижня» — **чек-лист** з прошлотижневих «Дій» по брендах (виконано/ні + причина).
- **Доступи як на борді:** менеджер — ні; РМ — свій регіон (+ мульти-регіон + тимчасові гранти); директор/admin — усі. Вибір регіону — glass-дропдаун (для одно-регіон РМ — статичний лейбл).

### ⚡ Власний зріз категорій клієнтів (швидкість + історія на дату)

1С віддає лише поточну категорію. Тепер тримаємо свій **SCD2-зріз** (`client_category_history`):

- Погодинний крон (`/api/cron/sync-client-categories`) обходить усіх менеджерів через 1С Action 8 (категорія + резерв в одному виклику), версіонує зміни (category/manager/region), оновлює резерв, закриває зниклих — **тільки серед менеджерів, що відповіли** (упавший не обнуляє своїх клієнтів).
- `region-stats` читає ростер із БД → гарячий шлях **3 виклики 1С/менеджера → 1** (лишається тільки факт). Fallback на live, поки зріз порожній.

### 📈 Стратегія — live-синк продажів (без 1С при кожному відкритті)

Дашборд «Стратегія» читав `sales_kpi_rollup`, але поточний місяць треба тримати живим:

- Крон (`/api/cron/sync-sales`, кожні 30 хв) тягне поточний місяць з 1С **`getSalesLineItems`** (line-items), класифікує нашою логікою (бренд/канал/подарунок), робить full-month replace у `sales` + `refresh_kpi_rollup`. У перші 10 днів місяця дотягує ще й **попередній** місяць (пізні правки при закритті).
- `?month=YYYY-MM` (admin) — пересинк будь-якого місяця для сверки/добору.

### 🗄️ Міграції

- `054_client_category_history` — SCD2-зріз категорій клієнтів.
- `055_weekly_report_notes` — замітки звіту (append-only, понедельно).
